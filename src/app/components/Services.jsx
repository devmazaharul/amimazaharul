...
export default function Services() {
  const {
    services_section: { title, desc, services },
    colorSchima,
  } = site_data;
    ...
    <div className="py-16 px-4 bg-white rounded-xl  mb-4 ">
      <div>
        <p className="flex items-center gap-1 border rounded-full w-fit px-2 py-1 text-sm uppercase text-gray-500 mb-6 ">
          services
          <MdOutlineCleaningServices className="text-[#00bc91]" />
        </p>
      </div>
        ...
        <div className="md:col-span-7">
          ...
          <div className="my-4 grid md:grid-cols-2 grid-cols-1  ">
            {services.map((item) => (
              <div
                key={Math.random()}
                className=" md:m-2 my-2 text-gray-500 border rounded-md mx-auto "
              >
                <div className=" block  rounded-md px-4 py-6 text-left leading-6">
                  <p className="pb-2">
                    {item.iconName == "web_design" ? (
                      <SiAntdesign
                        className={`text-[${colorSchima.text_color}] text-2xl`}
                      />
                    ) : item.iconName == "web_development" ? (
                      <FaNodeJs
                        className={`text-[${colorSchima.text_color}] text-2xl`}
                      />
                    ) : item.iconName == "grapich_design" ? (
                      <MdGraphicEq
                        className={`text-[${colorSchima.text_color}] text-2xl`}
                      />
                    ) : item.iconName == "digital_marketing" ? (
                      <TbBrandBumble
                        className={`text-[${colorSchima.text_color}] text-2xl`}
                      />
                    ) : (
